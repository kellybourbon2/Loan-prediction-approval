...
class DataPreprocessor:
    ...
    def feature_preprocessing(self, X_train: pd.DataFrame, X_test: pd.DataFrame):
        """Transform categorical columns so they can be processed by the model"""

        # Ordinal encoding of 'age_category' — fit on train, transform both
        self.ordinal_encoder = OrdinalEncoder(categories=[CATEGORIES_AGE])
        X_train["age_category"] = self.ordinal_encoder.fit_transform(
            X_train[["age_category"]]
        )
        X_test["age_category"] = self.ordinal_encoder.transform(
            X_test[["age_category"]]
        )
    ...
    def preprocessing_pipeline(self, df: pd.DataFrame):
        ...
        X_train, X_test = self.feature_preprocessing(X_train, X_test)
